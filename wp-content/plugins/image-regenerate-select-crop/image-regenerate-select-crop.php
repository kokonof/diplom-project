...
define( 'SIRSC_ASSETS_VER', '20230408.1832' );
...
class SIRSC_Image_Regenerate_Select_Crop {
	...
	public static function one_time_custom_image_size_names_choose( $list ) { // phpcs:ignore
		if ( ! did_action( 'sirsc/image_size_names_choose' ) ) {
			do_action( 'sirsc/image_size_names_choose', $list );
		}
	...
	public static function image_editor( $id, $file, $name = '', $info = [], $small_crop = '', $force_quality = 0 ) { //phpcs:ignore
		...
		if ( ! empty( $estimated ) &&
			...
			} elseif ( ! empty( $meta['width'] ) && $estimated[0] === $meta['width']

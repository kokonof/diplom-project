...
function log_init( string $name = 'tracer' ): void {
	...
	$file = $path . '/' . \esc_attr( $name ) . '-last.log';
	if ( ! is_file( $file ) ) {
		$fs->mkdir( $path );
		$fs->touch( $file );
	}
...
function log_read_last( string $log = 'bulk' ): string {
	$fs   = fs();
	$file = SIRSC_PLUGIN_DIR . 'log/' . \esc_attr( $log ) . '-last.log';
	if ( is_file( $file ) ) {
...
function log_write_last( string $log = 'bulk', string $text = '' ) {
	if ( ! empty( $text ) ) {
		log_init( $log );
		$fs = fs();
		$fs->put_contents( SIRSC_PLUGIN_DIR . 'log/' . $log . '-last.log', $text );
	}
}
...
function generic_log_write( $log = 'bulk', $ob = '', $prepend = true ) {
	if ( ! empty( $ob ) ) {
		...
		$fs = fs();
		if ( true === $prepend ) {
			$fs->put_contents( SIRSC_PLUGIN_DIR . 'log/' . $log . '.log', $text . log_read( $log ) );
		} else {
			$fs->put_contents( SIRSC_PLUGIN_DIR . 'log/' . $log . '.log', log_read( $log ) . $text );
		}
